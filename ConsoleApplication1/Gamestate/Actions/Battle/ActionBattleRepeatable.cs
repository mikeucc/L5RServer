...
{
    public class ActionBattleRepeatable:IAction
    {
        public override bool canBePerformed()
        {
            if (gs.currentBattlefield.myCards(this.actionOwner.playerOwner).Count == 0 || this.actionOwner.IsBowed || !gs.isBattlePhase)
            {
